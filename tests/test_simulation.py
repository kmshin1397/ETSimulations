...
def test_simulation(tmpdir_factory, base_test_config, base_test_coords):
    ...
    sim = simulation.Simulation(
        ...
        str(test_base_coords_file),
        "new_test_tiltseries.mrc",
        "new_test_tiltseries_nonoise.mrc",
        0,
        str(subdir),
        apix=1,
        defocus=5,
...
def test_edit_output_files(test_simulation):
    ...
    with open(test_simulation.config_file, "r") as f:
        ...
        f.readline()
        assert (
            f.readline() == "log_file = %s/simulator.log\n" % test_simulation.temp_dir
        )
...
def test_create_particle_lists(
    test_simulation, mocker, mock_orientations, mock_particle_coordinates
):
    mock_particle_set = mocker.Mock()
    mock_particle_set.name = "test_name"
    mock_particle_set.key = True
    ...
    with open(test_simulation.config_file, "r") as f:
        ...
        expected_new_file = (
            initial_config_file + new_particle_segment + new_particle_set_segment
        )
...
def test_create_fiducials(test_simulation):
    ...
    mock_bead_occupancy = 0.00025
    initial_config_file = open(test_simulation.config_file, "r").read()

    test_simulation.create_fiducials(mock_bead, mock_bead_occupancy)
